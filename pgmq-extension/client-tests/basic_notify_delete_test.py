...
def test_pgmq_basic_delete_notification(db_connection: psycopg.Connection):
    """Test that deleting a message fires a notification on the DELETE channel."""
    now = int(time.time())
    queue_name = f"test_delete_notify_{now}"
    ...
    # send a message so we have something to delete
    message = {"hello": "world"}
    with db_connection.cursor() as cur:
    ...
    # disable delete notification subscription
    with db_connection.cursor() as cur:
    ...
    # send and delete a message should result in no notification
    with db_connection.cursor() as cur:
        cur.execute("SELECT pgmq.send(%s, %s)", (queue_name, json.dumps(message)))
        result = cur.fetchone()
        msg_id = result[0] if result else None
