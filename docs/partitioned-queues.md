...
`pgmq.metrics()` reports `default_partition_length`, the number of messages currently in a partitioned queue's default partition (`null` for queues that are not partitioned). Zero is the only healthy value. A non-zero value means maintenance for that queue is already failing, not that some messages are merely misfiled, so alert on it directly.

### Recovery

Move the messages out of the default partition into the partitions they belong to, then let maintenance catch up. `pg_partman` provides the procedure for this; run it outside a transaction block, because it commits as it goes:
